...
function NewBoxModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.qr_code.trim() || !form.label.trim()) {
      setError('Código QR e Nome são obrigatórios.');
      return;
    }
    setLoading(true);
    setError('');
    ...
    setLoading(false);
    if (err) {
      setError(err.message.includes('unique') ? 'Esse código QR já existe.' : err.message);
    } else {
      onSaved();
      onClose();
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Esse é o código que será impresso no adesivo QR.
            </p>
          </div>
          <div>
            ...
            <input
              className="w-full rounded-lg bg-white/10 px-3 py-2 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500"
              placeholder="Ex: Caixa 01 - Limpeza"
              value={form.label}
    ...
    <div className="min-h-screen bg-[#0f0f1a] p-4 text-white">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1 rounded-lg bg-purple-600 px-3 py-2 text-sm font-semibold hover:bg-purple-700"
        >
          <Plus size={16} />
          Nova caixa
        </button>
      ...
      {loading ? (
        ...
        <p className="mt-10 text-center text-gray-500">
          {search ? 'Nenhuma caixa encontrada.' : 'Nenhuma caixa cadastrada ainda.'}
        </p>
      ) : (
            ...
            >
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-3 text-xs text-gray-400">
                  ...
                  <span className="flex items-center gap-1">
                    <Package size={12} />
                    {box.total_produtos} produto(s) · {box.total_itens} item(s)
                  </span>
                </div>
              </div>
