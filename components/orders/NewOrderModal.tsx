...
export function NewOrderModal({ onClose, setNotification }: NewOrderModalProps) {
  ...
  const handleAddItem = async (product: Product) => {
    ...
    // If a supplier is selected, check for mappings
  ...
  const finalizeOrder = async () => {
    try {
      ...
      setNotification({ type: 'success', message: 'Pedido finalizado e exportado com sucesso!' });
      onClose();
    } catch (err: any) {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `Pedido_Compra_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  const exportToPDF = () => {
    const doc = new jsPDF();
    const supplierName = selectedSupplierId ? suppliers.find(s => s.id === selectedSupplierId)?.name : 'Sem Fornecedor';
    ...
    if (selectedSupplierId) {
      headers.push('SKU Fornec.', 'Ref Fornec.');
    }

    const tableData = orderItems.map(item => {
      const row = [item.ean, item.name, item.quantity];
      if (selectedSupplierId) {
        row.push(item.supplier_sku || '-', item.supplier_description || '-');
      }
      return row;
    });
    ...
    doc.save(`Pedido_Compra_${new Date().toISOString().split('T')[0]}.pdf`);
  };
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium tracking-wide uppercase">Criação de lista e exportação para fornecedor</p>
            </div>
          </div>
          <button 
            onClick={() => setShowConfirmCancel(true)}
        ...
        {showConfirmFinalize && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center">
              ...
              <h4 className="text-xl font-black text-slate-900 mb-2 uppercase">Finalizar Pedido?</h4>
              <p className="text-sm text-slate-500 font-medium mb-8">O sistema irá salvar o histórico e gerar os arquivos Excel e PDF para download.</p>
              <div className="grid grid-cols-2 gap-4">
                ...
                <button onClick={finalizeOrder} className="py-3 font-bold text-white bg-green-600 rounded-xl hover:bg-green-700 transition-colors uppercase text-xs shadow-lg shadow-green-600/20">Finalizar</button>
              </div>
            </motion.div>
          </div>
        )}
