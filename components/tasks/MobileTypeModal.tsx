...
import { X, ClipboardList, CheckSquare, LayoutGrid, Printer } from 'lucide-react';
...
export function MobileTypeModal({
  ...
  const cards = [
    ...
    },
    {
      ...
      desc: 'Solicitar impressão de etiquetas',
      onClick: onSelectFilaImpressao,
      highlight: true,
    },
  ];
                ...
                <div className="grid grid-cols-3 gap-3.5">
                  {cards.map(c => (
                    <button
                      ...
                      onClick={c.onClick}
                      className={cn(
                        'group flex flex-col items-start gap-3 p-5 rounded-2xl text-left transition-colors relative',
                        c.highlight
                          ? 'bg-primary/[0.03] dark:bg-primary/[0.06] border-2 border-primary/40 hover:border-primary/60'
                          : 'bg-surface border border-black/[0.08] dark:border-white/[0.07] hover:border-primary/40 hover:bg-primary/[0.03]'
                      )}
                    >
                      {c.highlight && (
                        <span className="absolute top-3 right-3 text-[8px] font-black uppercase tracking-wide px-1.5 py-0.5 rounded-full bg-primary text-white">Novo</span>
                      )}
                      <div className={cn(
                        'w-11 h-11 rounded-xl flex items-center justify-center transition-colors',
                        c.highlight
                          ? 'bg-primary/10 text-primary'
                          : 'bg-black/[0.05] dark:bg-white/[0.06] text-on-surface/50 group-hover:bg-primary/10 group-hover:text-primary'
                      )}>
                        <c.icon size={22} />
                      </div>
                      <div>
                        <p className={cn(
                          ...
                          c.highlight ? 'text-primary' : 'text-on-surface group-hover:text-primary'
                        )}>
                          {c.title}
                        </p>
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6">
            ...
            {cards.map(c => (
              <button
                ...
                onClick={c.onClick}
                className={cn(
                  'w-full max-w-sm rounded-2xl p-5 flex items-center gap-4 text-left active:scale-95 transition-all group relative',
                  c.highlight
                    ? 'bg-[#D81E1E]/[0.04] dark:bg-[#D81E1E]/[0.08] border-2 border-[#D81E1E]/50'
                    : 'bg-white dark:bg-[#252520] border-2 border-[#E0D8BF] dark:border-white/[0.08] hover:border-[#D81E1E]/40 hover:bg-[#D81E1E]/[0.02]'
                )}
              >
                {c.highlight && (
                  <span className="absolute top-3 right-3 text-[8px] font-black uppercase tracking-wide px-1.5 py-0.5 rounded-full bg-[#D81E1E] text-white">Novo</span>
                )}
                <div className={cn(
                  'w-12 h-12 rounded-xl flex items-center justify-center shrink-0 transition-colors',
                  c.highlight ? 'bg-[#D81E1E]/10' : 'bg-black/[0.05] dark:bg-white/[0.06] group-hover:bg-[#D81E1E]/10'
                )}>
                  <c.icon size={24} className={cn(c.highlight ? 'text-[#D81E1E]' : 'text-on-surface/50 group-hover:text-[#D81E1E] transition-colors')} />
                </div>
                <div>
                  <p className={cn(
                    'text-sm font-black leading-tight mb-0.5 transition-colors',
                    c.highlight ? 'text-[#D81E1E]' : 'text-on-surface group-hover:text-[#D81E1E]'
                  )}>
                    {c.title}
                  </p>
