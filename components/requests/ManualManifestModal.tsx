...
export function ManualManifestModal({
  ...
  // ── view: 'list' shows drafts landing; 'editor' shows the spreadsheet ──────
  ...
  // On open: load drafts and decide initial view
  useEffect(() => {
    if (!isOpen) return;
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          {linkedCnt > 0 && (
                            <span className="text-[10px] font-bold bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded">
                              {linkedCnt} vinculado{linkedCnt !== 1 ? 's' : ''}
                            </span>
                          )}
                        </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 overflow-hidden">
        ...
        >
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-4 bg-white shrink-0">
            {/* Back to list (only when drafts exist) */}
            {drafts.length > 0 && (
              <button
                onClick={goToList}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-400 font-medium mt-0.5">
                Preencha os itens e vincule ao dicionário do fornecedor
              </p>
            </div>
            ...
            <table className="w-full border-collapse min-w-[900px]">
              <thead className="sticky top-0 z-10 bg-slate-50">
                <tr className="border-b-2 border-slate-200">
                  ...
                  <th className="w-20 px-3 py-2.5 text-center text-[10px] font-black text-slate-400 uppercase tracking-wider border-r border-slate-200">Unid.</th>
