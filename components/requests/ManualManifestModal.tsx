...
export function ManualManifestModal({
  ...
  // Auto-lookup from supplier_mappings when leaving a field
  const lookupMapping = async (rowId: string, code: string, description: string) => {
    ...
    if (code.trim()) {
      q = q.eq('supplier_sku', code.trim());
    } else {
      q = q.ilike('supplier_description', `%${description.trim()}%`);
    }
    ...
    if (data?.products) {
      ...
      setRows(prev =>
        prev.map(r =>
          r.id === rowId
            ? { ...r, linkedProduct: { id: p.id, name: p.name, sku: p.sku, ean: p.ean ?? '' } }
            : r
        )
      );
    }
  ...
  const handleLinkSearch = async () => {
    ...
    const { data } = await supabase
      ...
      .or(
        `name.ilike.%${linkSearch}%,sku.ilike.%${linkSearch}%,ean.ilike.%${linkSearch}%`
      )
      .limit(8);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const fileName = `Manifesto Manual — ${timestamp}`;
      ...
      setNotification({
        type: 'success',
        message: `Manifesto enviado para revisão com ${items.length} item(s).`,
      });
      onClose();
    } catch (err: any) {
  ...
  const totalValue = rows.reduce((acc, r) => {
    return acc + (parseFloat(r.quantity) || 0) * (parseFloat(r.unitPrice) || 0);
  }, 0);
        ...
        >
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-4 bg-white shrink-0">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <button
                onClick={onClose}
                className="p-2 hover:bg-slate-100 rounded-xl transition-colors mt-4"
              >
            ...
            <table className="w-full border-collapse min-w-[900px]">
              <thead className="sticky top-0 z-10 bg-slate-50">
                <tr className="border-b-2 border-slate-200">
                  <th className="w-12 px-3 py-2.5 text-center text-[10px] font-black text-slate-400 uppercase tracking-wider border-r border-slate-200">
                    #
                  </th>
                  <th className="w-32 px-3 py-2.5 text-left text-[10px] font-black text-slate-400 uppercase tracking-wider border-r border-slate-200">
                    Código
                  </th>
                  <th className="min-w-[220px] px-3 py-2.5 text-left text-[10px] font-black text-slate-400 uppercase tracking-wider border-r border-slate-200">
                    Descrição
                  </th>
                  <th className="w-20 px-3 py-2.5 text-center text-[10px] font-black text-slate-400 uppercase tracking-wider border-r border-slate-200">
                    Unid.
                  </th>
                  <th className="w-28 px-3 py-2.5 text-right text-[10px] font-black text-slate-400 uppercase tracking-wider border-r border-slate-200">
                    Quantidade
                  </th>
                  <th className="w-36 px-3 py-2.5 text-right text-[10px] font-black text-slate-400 uppercase tracking-wider border-r border-slate-200">
                    Preço Unit.
                  </th>
                  <th className="w-36 px-3 py-2.5 text-right text-[10px] font-black text-slate-400 uppercase tracking-wider border-r border-slate-200">
                    Valor Total
                  </th>
                  <th className="min-w-[200px] px-3 py-2.5 text-left text-[10px] font-black text-slate-400 uppercase tracking-wider border-r border-slate-200">
                    Dicionário
                  </th>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
                ...
                <button
                  onClick={closeLink}
                  className="ml-3 p-2 hover:bg-slate-100 rounded-xl transition-colors"
                >
                      ...
                      <div className="space-y-1.5">
                        {linkResults.map(p => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-bold text-slate-800 truncate group-hover:text-primary">
                                {p.name}
                              </p>
                              <div className="flex items-center gap-2 mt-0.5">
                                ...
                                {p.ean && (
                                  <span className="text-[10px] text-slate-400">{p.ean}</span>
                                )}
                              </div>
                            </div>
                            <CheckCircle2
